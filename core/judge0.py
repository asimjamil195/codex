...
def _env_float(name: str, default: float) -> float:
    ...
    try:
        ...
        logger.warning(f"Invalid value for {name}. Falling back to default: {default}")
        return default
...
def _build_ssl_context() -> ssl.SSLContext:
    ...
    if ca_bundle_path:
        try:
            ...
            logger.warning("Failed to load Judge0 CA bundle '%s': %s", ca_bundle_path, exc)
...
LANGUAGE_DEFINITIONS: List[Mapping[str, object]] = [
    {"key": "python", "id": 71, "name": "Python (3.8.1)", "editor": "python", "aliases": ["py", "python3"]},
    {"key": "javascript", "id": 63, "name": "JavaScript (Node.js 12.14)", "editor": "javascript", "aliases": ["js", "node"]},
    {"key": "typescript", "id": 74, "name": "TypeScript (3.7.4)", "editor": "typescript", "aliases": ["ts"]},
    {"key": "c", "id": 50, "name": "C (GCC 9.2.0)", "editor": "c", "aliases": []},
    {"key": "cpp", "id": 54, "name": "C++ (GCC 9.2.0)", "editor": "cpp", "aliases": ["c++"]},
    {"key": "java", "id": 62, "name": "Java (OpenJDK 13)", "editor": "java", "aliases": []},
    {"key": "csharp", "id": 51, "name": "C# (Mono 6.6)", "editor": "csharp", "aliases": ["c#", "cs"]},
    {"key": "go", "id": 60, "name": "Go (1.13.5)", "editor": "go", "aliases": ["golang"]},
    {"key": "rust", "id": 73, "name": "Rust (1.40.0)", "editor": "rust", "aliases": []},
    {"key": "ruby", "id": 72, "name": "Ruby (2.7.0)", "editor": "ruby", "aliases": []},
    {"key": "php", "id": 68, "name": "PHP (7.4.1)", "editor": "php", "aliases": []},
    {"key": "swift", "id": 83, "name": "Swift (5.2.3)", "editor": "swift", "aliases": []},
    {"key": "kotlin", "id": 78, "name": "Kotlin (1.3.70)", "editor": "kotlin", "aliases": []},
    {"key": "sql", "id": 82, "name": "SQL (SQLite 3.27)", "editor": "sql", "aliases": ["sqlite"]},
    {"key": "bash", "id": 46, "name": "Bash (5.0.0)", "editor": "shell", "aliases": ["sh", "shell"]},
]
...
def _resolve_language(language: str) -> Mapping[str, object]:
    ...
    if not entry:
        raise ValueError(f"Unsupported language '{language}'. Supported languages: {', '.join(_LANGUAGE_ALIAS_INDEX.keys())}.")
    return entry
...
def _perform_request(
    ...
    *,
    ...
    try:
        with urllib_request.urlopen(req, timeout=REQUEST_TIMEOUT, context=_SSL_CONTEXT) as response:
            content = response.read().decode("utf-8")
            return json.loads(content) if content else {}
    except urllib_error.HTTPError as exc:
        error_content = exc.read().decode("utf-8") if exc.read() else ""
        ...
        if isinstance(exc.reason, ssl.SSLCertVerificationError):
            raise Judge0Error(
                ...
                f"{exc.reason}. Configure JUDGE0_CA_BUNDLE_PATH to trust your "
                "certificate authority or set JUDGE0_DISABLE_SSL_VERIFY=1 "
                "for local testing."
            ) from exc
        raise Judge0Error(f"Judge0 connection error: {exc.reason}") from exc
